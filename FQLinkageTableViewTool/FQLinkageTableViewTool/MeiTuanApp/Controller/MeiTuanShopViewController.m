...
@interface MeiTuanShopViewController ()<FQGoodsNumberChangedDelegate,FQLinkageTableViewDelegate,CAAnimationDelegate>
...
@property (nonatomic, strong) FQLinkageTableView * goodsListView;
@property (nonatomic, strong) FQShoppingBar * shoppingBar;

@end

@implementation MeiTuanShopViewController

- (void)dealloc{
    NSLog(@"dealloc");
}

- (FQLinkageTableView *)goodsListView{
    if (!_goodsListView) {
        _goodsListView = [[FQLinkageTableView alloc]initWithFrame:CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height-[FQShoppingBar defaultHeight])];
...
- (FQShoppingBar *)shoppingBar{
    if (!_shoppingBar) {
        _shoppingBar = [FQShoppingBar barWithStyle:FQShoppingBarStyleDefault showInView:self.view];
...
- (void)initData{
    ...
    [arr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        FQGoodsGroupModel * goodsGroup = [FQGoodsGroupModel new];
        goodsGroup.classesName = [obj objectForKey:@"title"];
        NSArray * list = [obj objectForKey:@"list"];
        [list enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            FQGoodsModel * goods = [FQGoodsModel new];
            goods.goodsName = obj;
...
- (void)stepper:(MTShopCountStepper *)stepper valueChangedForCount:(NSInteger)count goods:(FQGoodsModel *)goods{
    if (stepper.style == FQCountStepperStyleGoodsList) {
        //更新购物车中的商品
